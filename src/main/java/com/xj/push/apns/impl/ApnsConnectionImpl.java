...
import static com.xj.push.apns.model.ApnsConstants.CHARSET_ENCODING;
import static com.xj.push.apns.model.ApnsConstants.ERROR_RESPONSE_BYTES_LENGTH;
import static com.xj.push.apns.model.ApnsConstants.PAY_LOAD_MAX_LENGTH;
...
import com.xj.push.apns.model.Command;
import com.xj.push.apns.model.ErrorResponse;
	...
	public void sendNotification(PushNotification notification) {
		...
		try {
			...
			if (plBytes.length > PAY_LOAD_MAX_LENGTH) {
				logger.error("Payload execeed limit, the maximum size allowed is 1000 bytes. " + payload);
				return;
			}
		...
		synchronized (lock) {
			...
			while (retries < maxRetries) {
				try {
					...
					logger.error(connName + " " + e.getMessage()+" retry: "+retries , e);
					closeSocket(socket);
					socket = null;
				}
				retries++;
			...
			if (!isSuccessful) {
				...
				logger.debug(String.format("%s Send success. count: %s, notificaion: %s", connName, notificaionSentCount.incrementAndGet(), notification));
				...
				try {
					...
					if (size == res.length && command == Command.ERROR) {
						...
						if (logger.isInfoEnabled()) {
							logger.debug(String.format("%s Received error response. status: %s, id: %s, error-desc: %s", connName, status, errorId, ErrorResponse.desc(status)));
						}
