...
public class ApnsConnectionPool implements Closeable {
	...
	private ApnsConnectionPool(ApnsConfig config, SocketFactory factory) {
		...
		for (int i = 0; i < poolSize; i++) {
			...
			connQueue.add(new ApnsConnectionImpl(this.factory, host, port, config.getRetries(), config.getCacheLength(), config.getName(), connName, config.getIntervalTime(), config.getTimeout()));
		}
	}
